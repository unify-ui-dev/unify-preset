...
const getGeneralShortcuts = () => {
	...
	const utils: Record<string, string> = {
		...
		"dropdown-content":"grid max-h-[inherit] grid-cols-[auto_1fr] overflow-y-auto overscroll-contain outline-hidden *:[[role='group']+[role=group]]:mt-4 *:[[role='group']+[role=separator]]:mt-1",
		...
		"dropdown-item-slot":"not-has-[[slot=description]]:items-center has-[[slot=description]]:**:data-[slot=check-indicator]:mt-[1.5px] **:data-[slot=avatar]:*:mr-1.5 **:data-[slot=avatar]:*:size-6 **:data-[slot=avatar]:mr-(--mr-icon) **:data-[slot=avatar]:size-6 sm:**:data-[slot=avatar]:*:size-5 sm:**:data-[slot=avatar]:size-5",
		"dropdown-item-icon":"*:slot-icon:mr-(--mr-icon) **:slot-icon:size-5 **:slot-icon:shrink-0 sm:**:slot-icon:size-4",
		"dropdown-item-label":"[&>[slot=label]+[data-slot=icon]]:absolute [&>[slot=label]+[data-slot=icon]]:right-1",
		"dropdown-item-force-color":"forced-color-adjust-none forced-colors:text-[CanvasText] forced-colors:**:slot-icon:text-[CanvasText] forced-colors:group-focus:**:slot-icon:text-[CanvasText]"
	};
