...
const getInfoSizes = (size: switchSize) => {
    ...
    if (!sizeS) return ''
    return `checked:before:translate-x-${getConfigValue(sizeS.translateX)} before:left-${getConfigValue(sizeS.left)} w-${getConfigValue(sizeS.width)} h-${getConfigValue(sizeS.height)} before:size-${getConfigValue(sizeS.indicatorSize)}`
}
const getSwitchShortcuts = ({ appearance }: { appearance: Appearance }) => {
    ...
    const switchs = {
        'ct-ui-before-switch': ``,
        'switch': `appearance-none focus-visible:ring-2 focus-visible:ring-current focus:ring-0 focus:ring-transparent focus:ring-offset-transparent ${getRingOffsetBg(appearance)}
    ...
    const dynamicSwitchs: [RegExp, (params: RegExpExecArray) => string][] = [
